 ...
 *
 * Commands: demo | build <dir> | validate <dir> | init <dir> | list | help
 */
import { promises as fs } from "node:fs";
import path from "node:path";
import { parseScoreIr, validateScore, type ScoreValidationResult } from "@worship-score/core";
...
async function readScoreIr(scoreDir: string) {
...
async function cmdBuild(scoreDir: string): Promise<void> {
  ...
  const { options } = await loadBuildOptions(scoreDir);
  console.log(
    `  옵션: 코드 ${options.chords?.visible ? "표시" : "숨김"} · 전조 ${options.key?.transposeSemitones ?? 0}반음 · 배경 ${options.background ? "있음" : "없음"}`,
  );
  ...
  await fs.writeFile(path.join(outDir, "score.musicxml"), result.musicXml, "utf8");
  ...
  console.log(`  슬라이드 ${result.pptx.slideCount}장 · OOXML 검증 ${result.validation.ok ? "통과 ✓" : `실패 ✗ (${failed.map((f) => f.name).join(", ")})`}`);
...
const OPTIONS_TEMPLATE = {
  ...
  background: { image: "background.png", scrim: 0.82 },
  style: {
...
async function cmdDemo(): Promise<void> {
  ...
  const demoOptions = {
    ...
    background: { image: "background.png", scrim: 0.84 },
    style: {
